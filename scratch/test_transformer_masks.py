...
class TransformerEncoderModel(nn.Module):
    ...
    def forward(self, x, lengths=None):
        ...
        # BOOLEAN MASKS FOR BOTH TO AVOID WARNING
...
def run_test(model, name):
    ...
    if torch.allclose(rep1_T2, rep2_T2):
        print(f"CAUSALITY PROVEN ({name}): T=2 output is IDENTICAL despite future T=5 being modified.")
    else:
        print(f"LEAKAGE DETECTED ({name}): T=2 output changed when future T=5 was modified.")

torch.manual_seed(42)
run_test(TransformerEncoderModel(), "TRANSFORMER ENCODER")
run_test(TemporalTransformerModel(), "TEMPORAL TRANSFORMER")
