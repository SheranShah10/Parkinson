...
| **FTTransformer** | 0.581793 | 0.760550 |
...
| **SAINT** | 8.86789 | 0.615280 |
| **TabNet** | 9.44636 | 0.565670 |
...
## Tier 4: Ensembles (Fusing Static & Temporal Predictions)

To squeeze out the absolute maximum performance, we combined the predictions of the best models from Tiers 1-3. We mathematically aligned the Tabular representations (used by CatBoost/GBDT) and the dynamic Sequence tensors (used by the GRU) so they were perfectly synchronized on the exact same cross-validation folds.

### **Regression Leaderboard (NP3TOT Severity)**
| Model | RMSE | $R^2$ Score |
| :--- | :--- | :--- |
| **Voting Ensemble (GBDT + CatBoost + GRU)** | **8.4791** | **0.6959** |
| **Stacking Ensemble (Meta-Model)** | 8.4796 | 0.6959 |
| *Base GBDT* | 8.6416 | 0.6842 |
| *Base CatBoost* | 8.6888 | 0.6807 |
| *Base GRU* | 9.3125 | 0.6332 |

> [!TIP]
> The Ensemble successfully broke the $0.694\ R^2$ ceiling established by CatBoost in Tier 1! Fusing the sequential trajectory predictions of the GRU with the flat clinical features of the Gradient Boosted Trees yielded the best model in the entire pipeline.
